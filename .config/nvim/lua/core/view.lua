...
function M.register(name, opts)
	...
	-- map("n", opts.map, ':lua require("view").toggle("' .. name .. '")<CR>', { noremap = true, silent = true })
	vim.keymap.set("n", opts.map, ':lua require("core.view").toggle("' .. name .. '")<CR>', { noremap = true, silent = true })
